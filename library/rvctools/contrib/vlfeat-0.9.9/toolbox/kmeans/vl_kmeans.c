...
printf_func_t  my_printf_func(char const *fmt, ...);
...
mexFunction (int nout, mxArray * out[], int nin, const mxArray * in[])
{
  ...
  vl_set_printf_func((printf_func_t) my_printf_func);
...
/*
 * we need to flush the messages to stdout, let's define our own printf-like function which does the business.
 */
printf_func_t
my_printf_func(char const *fmt, ...)
{
    char    buf[BUFSIZ];
    va_list ap;
    ...
    vsnprintf(buf, BUFSIZ, fmt, ap);   // we really need mexVPrintf
    mexPrintf("%s", buf);
    mexEvalString("drawnow");  // and this is how we flush the buffer
}
